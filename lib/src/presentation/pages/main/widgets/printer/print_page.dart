...
class _PrintPageState extends State<PrintPage> {
  ...
  Future _printReceiveTest() async {
    ...
    subTotal = ((widget.orderData?.totalPrice ?? 0) -
        ...
        (widget.orderData?.serviceFee ?? 0) - (widget.orderData?.totalDiscount ?? 0));
    ...
    bytes += generator.row([
      ...
      PosColumn(
        width: 6,
        text: widget.orderData?.username ?? "",
      ...
      for (int i = 0;
        ...
        bytes += generator.row([
          PosColumn(
            width: 10,
            text:
                "$customSpace${addons.stocks?.product?.translation?.title ?? ""} ( ${NumberFormat.currency(
              symbol: widget.orderData?.currency?.symbol ?? "",
    ...
    bytes += generator.row([
      ...
      PosColumn(
        width: 6,
        text: AppHelpers.numberFormat(widget.orderData?.tax),
        styles: const PosStyles(align: PosAlign.right, bold: true),
      ),
    ]);
    bytes += generator.row([
      PosColumn(
        width: 6,
        text: customSpace + AppHelpers.getTranslation(TrKeys.serviceFee),
        styles: const PosStyles(align: PosAlign.left, bold: true),
      ),
      PosColumn(
        width: 6,
        text: AppHelpers.numberFormat(widget.orderData?.serviceFee),
        styles: const PosStyles(align: PosAlign.right, bold: true),
      ),
    ]);
    ...
    bytes += generator.row([
      ...
      PosColumn(
        width: 6,
        text: AppHelpers.numberFormat(widget.orderData?.totalDiscount),
        styles: const PosStyles(align: PosAlign.right, bold: true),
      ),
    ]);
    bytes += generator.row([
      ...
      PosColumn(
        width: 6,
        text: AppHelpers.numberFormat(widget.orderData?.totalPrice),
        styles: const PosStyles(align: PosAlign.right, bold: true),
      ),
    ]);
    bytes += generator.text(customDivider);
